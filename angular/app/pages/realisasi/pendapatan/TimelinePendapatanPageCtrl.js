...
(function () {
    ...
    angular.module('BlurAdmin.pages.realisasi')
        .controller('TimelinePendapatanPageCtrl', TimelinePendapatanPageCtrl);

    /** @ngInject */
    function TimelinePendapatanPageCtrl($scope, $uibModal, $log, $timeout, moment, RPJM, WaktuPelaksanaan, $q, AnggaranPendapatan, Pendapatan) {
            ...
            var promises = waktuPelaksanaanList.map(function (waktupelaksanaan) {
                ...
                Pendapatan.find({
                    ...
                    angular.forEach(result, function (item, index, arr) {
                        ...
                        var tasks = [];
                        var subPendapatanList = item.SubPendapatan;
                        if (subPendapatanList.length > 0) {
                            subPendapatanList.forEach(function (subItem) {
                                var task = {
                                    id: subItem.id,
                                    name: subItem.Nama,
                                    priority: 2,
                                ...
                                parentGantt.children.push(subItem.Nama)
                            })

                        }

                        ganttData.push(parentGantt);
                    })
        ...
        $scope.open = function (page, size, RKPId) {
            var modalInstance = $uibModal.open({
                ...
                templateUrl: page,
                size: size,
                controller: TimelinePendapatanModalInstanceCtrl,
                controllerAs: 'vm',
                resolve: {
        ...
        $scope.handleTaskIconClick = function (taskModel) {
            ...
            if (taskModel.priority == 1) $scope.open('app/pages/realisasi/pendapatan/modal.html', '', taskModel.RKPId);
        };
    ...
    angular.module('BlurAdmin.pages.realisasi')
        .controller('TimelinePendapatanModalInstanceCtrl', TimelinePendapatanModalInstanceCtrl);

    function TimelinePendapatanModalInstanceCtrl($uibModalInstance, selectedTask, selectedRKP, $state) {
        var vm = this;
